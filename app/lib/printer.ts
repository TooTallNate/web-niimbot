...
export class PrinterClient {
    ...
    static async connect(deviceNamePrefix: string): Promise<PrinterClient> {
        ...
        for (const char of characteristics) {
            ...
            if (properties.read && properties.writeWithoutResponse && properties.notify) {
                characteristic = char;
                break;
            }
        }
        if (!characteristic) throw new Error("Required characteristic not found");
    ...
    private async sendCommand(requestCode: RequestCodeEnum, data: Uint8Array): Promise<NiimbotPacket> {
        ...
        const responsePromise = new Promise<NiimbotPacket>((resolve, reject) => {
            ...
            characteristic.addEventListener('characteristicvaluechanged', (event) => {
                ...
                const response = NiimbotPacket.fromBytes(new Uint8Array(value.buffer));
                resolve(response);
            }, { once: true });
        });
    ...
    _encodeLine(lineNumber: number, line: Bit[]) {
        ...
        header[4] = 0;
        header[5] = 1;
        const packet = new NiimbotPacket(0x85, new Uint8Array([
            ...header,
            ...binaryStringToBytes(line.join(""))
        ]));
        return packet;
    }

    async heartbeat() {
        const packet = await this.sendCommand(RequestCodeEnum.HEARTBEAT, new Uint8Array([1]));
    ...
    async startPrint(): Promise<boolean> {
        const response = await this.sendCommand(RequestCodeEnum.START_PRINT, new Uint8Array([1]));
        return Boolean(response.data[0]);
    }

    async endPrint(): Promise<boolean> {
        const response = await this.sendCommand(RequestCodeEnum.END_PRINT, new Uint8Array([1]));
        return Boolean(response.data[0]);
    }

    async startPagePrint(): Promise<boolean> {
        const response = await this.sendCommand(RequestCodeEnum.START_PAGE_PRINT, new Uint8Array([1]));
        return Boolean(response.data[0]);
    }

    async endPagePrint(): Promise<boolean> {
        const response = await this.sendCommand(RequestCodeEnum.END_PAGE_PRINT, new Uint8Array([1]));
        return Boolean(response.data[0]);
    }

    async allowPrintClear(): Promise<boolean> {
        const response = await this.sendCommand(RequestCodeEnum.ALLOW_PRINT_CLEAR, new Uint8Array([1]));
        return Boolean(response.data[0]);
    }
    ...
    async getPrintStatus(): Promise<{ page: number, progress1: number, progress2: number }> {
        const response = await this.sendCommand(RequestCodeEnum.GET_PRINT_STATUS, new Uint8Array([1]));
